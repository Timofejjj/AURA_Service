...
export interface SoftenRequirementsEntryData {
  rigidRequirement: string;
  whyMandatory: string;
  rationalPreference: string;
  newThought: string;
  timestamp: string;
}

interface SoftenRequirementsEntryCardProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: SoftenRequirementsEntryData) => void;
}
...
export function SoftenRequirementsEntryCard({ isOpen, onClose, onSave }: SoftenRequirementsEntryCardProps) {
  const [rigidRequirement, setRigidRequirement] = useState('');
  const [whyMandatory, setWhyMandatory] = useState('');
  const [rationalPreference, setRationalPreference] = useState('');
  ...
  const handleSave = () => {
    onSave({
      rigidRequirement: rigidRequirement.trim(),
      whyMandatory: whyMandatory.trim(),
      rationalPreference: rationalPreference.trim(),
      newThought: newThought.trim(),
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          key="soften-requirements-entry-overlay"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          ...
          <motion.div
            role="dialog"
            aria-modal="true"
            aria-labelledby="soften-requirements-card-title"
          ...
          >
            ...
            <button
              ...
              style={{ right: 8, top: 8 }}
              aria-label="Сохранить запись Смягчение требований"
            >
              <Check size={22} strokeWidth={2.5} />
            </button>

            <div className="px-5 pt-14 pb-6 space-y-4">
              {fieldBlock('Жесткое требование', rigidRequirement, setRigidRequirement, 'Что «должно» быть?', true, firstInputRef)}
              {fieldBlock('Почему я считаю, что это обязательно?', whyMandatory, setWhyMandatory, 'Объясните…', true)}
              {fieldBlock('Рациональное предпочтение', rationalPreference, setRationalPreference, 'Что было бы желательно, но не обязательно?', true)}
              {fieldBlock('Новая мысль', newThought, setNewThought, 'Я предпочитаю это, но могу пережить, если этого не будет', true)}
            </div>
          </motion.div>
        </motion.div>
      )}
