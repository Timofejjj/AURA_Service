...
export interface ProtocolEntryData {
  situation: string;
...
interface ProtocolEntryCardProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: ProtocolEntryData) => void;
}
...
export function ProtocolEntryCard({ isOpen, onClose, onSave }: ProtocolEntryCardProps) {
  ...
  const fieldBlock = (
    ...
    <div className="flex gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className={hintClass}>Одно-два предложения.</p>
      </div>
    </div>
  );
      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            role="dialog"
            aria-modal="true"
            aria-labelledby="protocol-card-title"
          ...
          >
            ...
            <button
              ...
              style={{ right: 8, top: 8 }}
              aria-label="Сохранить запись Протокол мыслей"
            >
              <Check size={22} strokeWidth={2.5} />
            </button>

            <div className="px-5 pt-14 pb-6 space-y-4">
              {fieldBlock('Ситуация', situation, setSituation, 'Опишите ситуацию…', false, firstInputRef)}
              {fieldBlock('Автоматическая мысль', automaticThought, setAutomaticThought, 'Какая мысль пришла первой?', true)}
              {fieldBlock('Эмоция', emotion, setEmotion, 'Какая эмоция возникла?', false)}
              {fieldBlock('Доказательства «за»', evidenceFor, setEvidenceFor, 'Факты в пользу автоматической мысли…', true)}
              {fieldBlock('Доказательства «против»', evidenceAgainst, setEvidenceAgainst, 'Факты против этой мысли…', true)}
              {fieldBlock('Альтернативная мысль', alternativeThought, setAlternativeThought, 'Более сбалансированная мысль…', true)}
            </div>
          </motion.div>
        </motion.div>
      )}
