...
export interface ThoughtDisputeEntryData {
  irrationalThought: string;
  logicalQuestion: string;
  empiricalQuestion: string;
  pragmaticQuestion: string;
  rationalAlternative: string;
  timestamp: string;
}

interface ThoughtDisputeEntryCardProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: ThoughtDisputeEntryData) => void;
}
...
export function ThoughtDisputeEntryCard({ isOpen, onClose, onSave }: ThoughtDisputeEntryCardProps) {
  const [irrationalThought, setIrrationalThought] = useState('');
  const [logicalQuestion, setLogicalQuestion] = useState('');
  const [empiricalQuestion, setEmpiricalQuestion] = useState('');
  const [pragmaticQuestion, setPragmaticQuestion] = useState('');
  const [rationalAlternative, setRationalAlternative] = useState('');
  const firstInputRef = useRef<HTMLTextAreaElement>(null);

  useEffect(() => {
    if (isOpen) {
      setIrrationalThought('');
      setLogicalQuestion('');
      setEmpiricalQuestion('');
      setPragmaticQuestion('');
  ...
  const handleSave = () => {
    onSave({
      irrationalThought: irrationalThought.trim(),
      logicalQuestion: logicalQuestion.trim(),
      empiricalQuestion: empiricalQuestion.trim(),
      pragmaticQuestion: pragmaticQuestion.trim(),
      rationalAlternative: rationalAlternative.trim(),
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
          key="thought-dispute-entry-overlay"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          ...
          <motion.div
            role="dialog"
            aria-modal="true"
            aria-labelledby="thought-dispute-card-title"
          ...
          >
            ...
            <button
              ...
              style={{ right: 8, top: 8 }}
              aria-label="Сохранить запись Спор с мыслью"
            >
              <Check size={22} strokeWidth={2.5} />
            </button>

            <div className="px-5 pt-14 pb-6 space-y-4">
              {fieldBlock('Иррациональная мысль', irrationalThought, setIrrationalThought, 'Во что я верю?', true, firstInputRef)}
              {fieldBlock('Логический вопрос', logicalQuestion, setLogicalQuestion, 'Есть ли логика в этой мысли?', true)}
              {fieldBlock('Эмпирический вопрос', empiricalQuestion, setEmpiricalQuestion, 'Какие есть реальные доказательства?', true)}
              {fieldBlock('Прагматический вопрос', pragmaticQuestion, setPragmaticQuestion, 'Помогает ли эта мысль мне?', true)}
              {fieldBlock('Рациональная альтернатива', rationalAlternative, setRationalAlternative, 'Какую мысль лучше использовать?', true)}
            </div>
          </motion.div>
        </motion.div>
      )}
